...
KellyTools.val = function(value, type) {
    ...
    if (type == 'string') {
        ...
        if (!value) return '';
        return value.replace(/[^Ğ°-ÑĞ-Ğ¯a-z0-9 ._-]/gim, "_");
    } 
}
...
KellyTools.getUrlFileName = function(url, excludeExt, noDecode) {
    ...
    if (!noDecode && url.indexOf('%') != -1) {
        ...
        url = url.replace(/[^Ğ°-ÑĞ-Ğ¯a-z0-9Ã¡Ã©Ã­Ã³ÃºÃ±Ã¼ ._-]/gim, "");
    } 
...
KellyTools.generateIdWord = function(text) {
    ...
    var replaceFrom = ['Ğ°','Ğ±','Ğ²','Ğ³','Ğ´','Ğµ','Ñ‘','Ğ¶','Ğ·','Ğ¸','Ğ¹','Ğº','Ğ»','Ğ¼','Ğ½','Ğ¾','Ğ¿','Ñ€','Ñ','Ñ‚','Ñƒ','Ñ„','Ñ…','Ñ†','Ñ‡', 'Ñˆ', 'Ñ‰', 'ÑŠ','Ñ‹','ÑŒ','Ñ', 'Ñ', 'Ñ',' ', '-'];
...
KellyTools.setCopyright = function(id, context) {
    ...
    if (context == 'popup') html = '<a target="_blank" href="/env/html/recorderDownloader.html?tab=help">' + KellyLoc.s('', 'help') + '</a>Â© <a target="_blank" href="https://github.com/Remilya/DeksmoScrap">' + KellyLoc.s('', 'nradiowave_person') + '</a>';
    else html = 'created by <a target="_blank" href="https://github.com/Remilya/DeksmoScrap">' + KellyLoc.s('', 'nradiowave_person')  + '</a>';
...
// todo - Ğ¿Ñ€ĞµĞ´ÑƒÑĞ¼Ğ¾Ñ‚Ñ€ĞµÑ‚ÑŒ Ğ¸Ğ½ÑÑ‚Ñ€ÑƒĞ¼ĞµĞ½Ñ‚Ñ‹ Ğ´Ğ»Ñ Ñ„Ğ¸ĞºÑĞ° 301 Ñ€ĞµĞ´Ğ¸Ñ€ĞµĞºÑ‚Ğ¾Ğ² ĞºĞ¾Ñ‚Ğ¾Ñ€Ñ‹Ğµ Ğ»ĞµĞ³ĞºĞ¾ ÑƒÑ‡ĞµÑÑ‚ÑŒ Ğ² webRequestĞ°Ñ… Ğ½Ğ¾ Ğ½ĞµĞ»ÑŒĞ·Ñ Ğ² declarativeNetRequest - Ğ²Ğ¾Ğ·Ğ¼Ğ¾Ğ¶Ğ½Ğ¾ Ğ¿Ğ¾Ğ´ÑÑ‚Ğ°Ğ½Ğ¾Ğ²ĞºĞ° referer Ñ‚Ğ°Ğº Ğ¶Ğµ ÑÑ€Ğ°Ğ±Ğ¾Ñ‚Ğ°ĞµÑ‚
// Ğ½Ğ°Ğ¿Ñ€Ğ¸Ğ¼ĞµÑ€ Ğ´Ğ¾Ğ±Ğ°Ğ²Ğ»ÑÑ‚ÑŒ Ğ² Ñ€ĞµĞ´Ğ¸Ñ€ĞµĞºÑ‚ ĞºĞ¾Ğ½Ñ‚Ñ€Ğ¾Ğ»ÑŒĞ½Ñ‹Ğ¹ get Ğ¿Ğ°Ñ€Ğ°Ğ¼ĞµÑ‚Ñ€ Ğ¸Ğ»Ğ¸ cookie Ğ¸ Ğ¿Ğ¾ Ğ½ĞµĞ¹ Ğ¾Ñ‚ÑĞµĞ¸Ğ²Ğ°Ñ‚ÑŒ Ğ´Ğ»Ñ Ğ¿Ñ€Ğ¾ÑÑ‚Ğ°Ğ½Ğ¾Ğ²ĞºĞ¸ referer (+2 Ğ¿Ñ€Ğ°Ğ²Ğ¸Ğ»Ğ° Ğ½Ğ° ĞºĞ°Ğ¶Ğ´ÑƒÑ ĞºĞ°Ñ€Ñ‚Ğ¸Ğ½ĞºÑƒ)
